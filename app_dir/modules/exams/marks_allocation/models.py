...
class ExamStatus(models.Model):
    academicclass = models.CharField(
        pgettext_lazy('ExamStatus field', 'academicclass'), max_length=128,
        null=True, blank=True)
    subject = models.CharField(
        pgettext_lazy('ExamStatus field', 'subject'), max_length=128,
        null=True, blank=True)
    academicyear = models.ForeignKey(AcademicYear, on_delete=models.CASCADE,
                                      null=True, blank=True)
    term = models.ForeignKey(Term, on_delete=models.CASCADE,
                                      null=True, blank=True)
    is_committed = models.BooleanField(
        pgettext_lazy('ExamStatus field', 'is_committed'),
        default=False)
    exam = models.CharField(
        pgettext_lazy('ExamStatus field', 'exam'), blank=True,
        null=True, max_length=128)
    updated_at = models.DateTimeField(
        pgettext_lazy('ExamStatus field', 'updated at'), auto_now=True, null=True)
    created = models.DateTimeField(pgettext_lazy('ExamConfiguration field', 'created'),
                                   default=now, editable=False)

    def __str__(self):
        return str(self.is_committed)
